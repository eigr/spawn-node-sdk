...
class ActorDeactivateStrategy$Type extends MessageType<ActorDeactivateStrategy> {
  ...
  create(value?: PartialMessage<ActorDeactivateStrategy>): ActorDeactivateStrategy {
    ...
    if (value !== undefined) reflectionMergePartial<ActorDeactivateStrategy>(this, message, value);
    return message;
  }
  internalBinaryRead(
    reader: IBinaryReader,
    length: number,
    options: BinaryReadOptions,
    target?: ActorDeactivateStrategy
...
class Actor$Type extends MessageType<Actor> {
  constructor() {
    super('eigr.functions.protocol.actors.Actor', [
      ...
      { no: 3, name: 'state', kind: 'message', T: () => ActorState },
      {
        ...
        kind: 'message',
        T: () => ActorSnapshotStrategy
      },
      {
        no: 5,
        name: 'deactivate_strategy',
        kind: 'message',
        T: () => ActorDeactivateStrategy
      }
    ]);
  }
    ...
    while (reader.pos < end) {
      ...
      switch (fieldNo) {
        ...
        case /* eigr.functions.protocol.actors.ActorDeactivateStrategy deactivate_strategy */ 5:
          message.deactivateStrategy = ActorDeactivateStrategy.internalBinaryRead(
            reader,
            reader.uint32(),
            options,
            message.deactivateStrategy
          );
          break;
        default:
          let u = options.readUnknownField;
  ...
  internalBinaryWrite(
    ...
    if (message.name !== '') writer.tag(1, WireType.LengthDelimited).string(message.name);
    /* bool persistent = 2; */
    if (message.persistent !== false) writer.tag(2, WireType.Varint).bool(message.persistent);
    ...
    if (message.deactivateStrategy)
      ActorDeactivateStrategy.internalBinaryWrite(
        message.deactivateStrategy,
