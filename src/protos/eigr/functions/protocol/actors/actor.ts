...
class FixedTimerCommand$Type extends MessageType<FixedTimerCommand> {
    ...
    create(value?: PartialMessage<FixedTimerCommand>): FixedTimerCommand {
        ...
        globalThis.Object.defineProperty(message, MESSAGE_TYPE, { enumerable: false, value: this });
        if (value !== undefined)
            reflectionMergePartial<FixedTimerCommand>(this, message, value);
        return message;
    }
    internalBinaryRead(reader: IBinaryReader, length: number, options: BinaryReadOptions, target?: FixedTimerCommand): FixedTimerCommand {
        let message = target ?? this.create(), end = reader.pos + length;
        while (reader.pos < end) {
            let [fieldNo, wireType] = reader.tag();
            switch (fieldNo) {
    ...
    internalBinaryWrite(message: FixedTimerCommand, writer: IBinaryWriter, options: BinaryWriteOptions): IBinaryWriter {
        /* int32 seconds = 1; */
        if (message.seconds !== 0)
            writer.tag(1, WireType.Varint).int32(message.seconds);
        /* eigr.functions.protocol.actors.Command command = 2; */
        if (message.command)
            Command.internalBinaryWrite(message.command, writer.tag(2, WireType.LengthDelimited).fork(), options).join();
        let u = options.writeUnknownFields;
        if (u !== false)
            (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
        return writer;
...
class Actor$Type extends MessageType<Actor> {
    constructor() {
        super("eigr.functions.protocol.actors.Actor", [
            ...
            { no: 4, name: "commands", kind: "message", repeat: 1 /*RepeatType.PACKED*/, T: () => Command },
            { no: 5, name: "timer_commands", kind: "message", repeat: 1 /*RepeatType.PACKED*/, T: () => FixedTimerCommand }
        ]);
    }
        ...
        while (reader.pos < end) {
            ...
            switch (fieldNo) {
                ...
                case /* repeated eigr.functions.protocol.actors.FixedTimerCommand timer_commands */ 5:
                    message.timerCommands.push(FixedTimerCommand.internalBinaryRead(reader, reader.uint32(), options));
                    break;
                default:
                    let u = options.readUnknownField;
                    if (u === "throw")
    ...
    internalBinaryWrite(message: Actor, writer: IBinaryWriter, options: BinaryWriteOptions): IBinaryWriter {
        ...
        for (let i = 0; i < message.timerCommands.length; i++)
            FixedTimerCommand.internalBinaryWrite(message.timerCommands[i], writer.tag(5, WireType.LengthDelimited).fork(), options).join();
        let u = options.writeUnknownFields;
        if (u !== false)
            (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
        return writer;
