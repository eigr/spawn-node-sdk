...
class UserState$Type extends MessageType<UserState> {
    ...
    create(value?: PartialMessage<UserState>): UserState {
        ...
        if (value !== undefined)
            reflectionMergePartial<UserState>(this, message, value);
        return message;
    }
...
class ChangeUserName$Type extends MessageType<ChangeUserName> {
    ...
    create(value?: PartialMessage<ChangeUserName>): ChangeUserName {
        ...
        globalThis.Object.defineProperty(message, MESSAGE_TYPE, { enumerable: false, value: this });
        if (value !== undefined)
            reflectionMergePartial<ChangeUserName>(this, message, value);
        return message;
    }
    internalBinaryRead(reader: IBinaryReader, length: number, options: BinaryReadOptions, target?: ChangeUserName): ChangeUserName {
        let message = target ?? this.create(), end = reader.pos + length;
        while (reader.pos < end) {
            let [fieldNo, wireType] = reader.tag();
            switch (fieldNo) {
    ...
    internalBinaryWrite(message: ChangeUserName, writer: IBinaryWriter, options: BinaryWriteOptions): IBinaryWriter {
        /* string new_name = 1; */
        if (message.newName !== "")
            writer.tag(1, WireType.LengthDelimited).string(message.newName);
        let u = options.writeUnknownFields;
        if (u !== false)
            (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
        return writer;
...
class ChangeUserNameResponse$Type extends MessageType<ChangeUserNameResponse> {
    constructor() {
        super("ChangeUserNameResponse", [
            { no: 1, name: "status", kind: "enum", T: () => ["ChangeUserNameStatus", ChangeUserNameStatus] },
